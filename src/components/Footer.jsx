...
export default function Footer() {
  return (
    <footer className="bg-white text-forest-900 border-t border-gray-200" role="contentinfo">
      {/* Main footer */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-2">
            <Link to="/" className="flex items-center gap-3 mb-4">
              ...
              />
              <div>
                <p className="text-forest-900 font-serif font-bold text-lg leading-tight">
                  Palamu Tiger Reserve
                </p>
                <p className="text-forest-600 text-xs tracking-widest uppercase">Official Journey Planner</p>
              </div>
            </Link>
          ...
          {Object.entries(links).map(([title, items]) => (
            <div key={title}>
              <h3 className="text-forest-900 font-semibold text-sm mb-4 tracking-wide">{title}</h3>
              <ul className="space-y-2">
                {items.map(({ label, to }) => (
                  <li key={label}>
                    <Link
                      to={to}
                      className="text-gray-600 hover:text-forest-700 text-sm transition-colors"
                    >
                      {label}
                    </Link>
                  </li>
      ...
      <div className="border-t border-gray-200 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <p className="text-xs text-gray-600 text-center">
            <strong className="text-forest-900">Important:</strong> Entry to PTR after 4:00 PM is strictly prohibited.
