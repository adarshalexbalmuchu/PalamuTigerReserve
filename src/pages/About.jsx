    ...
    <div className="flex gap-4">
      <div className="flex flex-col items-center">
        <div className={`w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${
          highlight ? 'bg-black text-white' : 'bg-neutral-100 text-neutral-700'
        }`}>
          {year}
        </div>
      ...
      <div className="pb-6 pt-2">
        <p className={`text-sm leading-relaxed ${highlight ? 'font-semibold text-neutral-800' : 'text-neutral-700'}`}>{event}</p>
      </div>
    </div>
  )
}
...
function TigerPopulationBar({ year, count, peak }) {
  ...
  const color = count >= 50 ? '#262626' : count >= 20 ? '#525252' : count >= 5 ? '#737373' : '#a3a3a3'
  return (
    <div className="flex items-center gap-3">
      <span className="text-xs text-neutral-500 w-10 text-right">{year}</span>
    ...
    <div className="min-h-screen bg-cream">
      ...
      <div className="bg-black text-white py-20 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-neutral-300 text-xl font-serif italic max-w-2xl">
      ...
      <section className="bg-white py-10 border-b border-neutral-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-neutral-100 gap-0">
            {[
              ...
              <div key={label} className="text-center p-6">
                <div className="font-serif font-bold text-2xl md:text-3xl text-neutral-800 mb-1">{value}</div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-16">
        ...
        <section>
          <h2 id="heritage" className="font-serif font-bold text-3xl text-neutral-800 mb-2">Conservation History</h2>
          ...
          <div className="max-w-2xl">
            {heritage.conservationMilestones.map((m) => (
              <TimelineItem
                key={m.year}
                year={m.year}
                event={m.event}
        ...
        <section className="bg-white rounded-2xl border border-neutral-200 p-8">
          <h2 className="font-serif font-bold text-2xl text-neutral-800 mb-2">Tiger Population Timeline</h2>
          ...
          <div className="mt-6 bg-neutral-50 border border-neutral-100 rounded-xl p-4 text-sm text-neutral-700">
            <strong className="text-neutral-800">Why this matters for your visit:</strong> Our itineraries
        ...
        {/* Wildlife */}
        <section id="wildlife">
          <h2 className="font-serif font-bold text-3xl text-neutral-800 mb-2">Wildlife & Biodiversity</h2>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {wildlife.map(a => (
              <div key={a.id} className="bg-white border border-neutral-100 rounded-2xl p-5">
                <div>
                  <h3 className="font-serif font-bold text-neutral-800">{a.name}</h3>
                  <p className="text-xs text-neutral-500 italic mb-2">{a.scientific} · {a.status}</p>
                  <p className="text-sm text-neutral-700 leading-relaxed mb-2">{a.description}</p>
                  <span className="text-xs bg-neutral-50 border border-neutral-200 text-neutral-700 px-2.5 py-1 rounded-full inline-block">
        ...
        <section className="bg-black text-white rounded-2xl p-8">
          <div className="mb-4">
            <div className="text-neutral-400 text-xs font-bold uppercase tracking-widest mb-1">India's Only Wolf Sanctuary</div>
          ...
          <div className="mt-6">
            <Link to="/plan" state={{ packageId: 'wolf-wilderness-5d' }} className="btn-primary">
        ...
        {/* Flora */}
        <section>
          <h2 className="font-serif font-bold text-3xl text-neutral-800 mb-2">Flora of PTR</h2>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            {flora.map(f => (
              <div key={f.name} className="bg-white border border-neutral-100 rounded-xl p-4">
                <h3 className="font-semibold text-neutral-800">{f.name}</h3>
                <p className="text-xs text-neutral-500 italic">{f.scientific}</p>
                <span className="text-[10px] bg-neutral-50 text-neutral-700 border border-neutral-200 px-2 py-0.5 rounded-full font-medium mt-1 inline-block">{f.role}</span>
                <p className="text-xs text-neutral-700 mt-2 leading-relaxed">{f.description}</p>
              </div>
            ))}
          </div>

          {/* Medicinal Plants */}
          <h3 className="font-serif font-bold text-xl text-neutral-800 mb-4">Traditional Medicinal Plants</h3>
              ...
              <tbody>
                {medicinalPlants.map((p, i) => (
                  <tr key={p.name} className={i % 2 === 0 ? 'bg-white' : 'bg-neutral-50'}>
                    ...
                    <td className="px-4 py-3 text-neutral-500 italic">{p.scientific}</td>
                    <td className="px-4 py-3 text-neutral-700">{p.uses}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* Palamu Forts */}
        <section>
          <h2 className="font-serif font-bold text-3xl text-neutral-800 mb-2">The Palamu Forts – Chero Dynasty</h2>
          <p className="text-neutral-600 mb-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-white border border-neutral-200 rounded-2xl p-6">
              <h3 className="font-serif font-bold text-xl text-neutral-800 mb-1">Old Fort</h3>
              <p className="text-xs text-neutral-500 mb-3">{heritage.palamuForts.oldFort.location}</p>
            ...
            <div className="bg-white border border-neutral-200 rounded-2xl p-6">
              <h3 className="font-serif font-bold text-xl text-neutral-800 mb-1">New Fort</h3>
              <p className="text-xs text-neutral-500 mb-3">{heritage.palamuForts.newFort.location}</p>
              ...
              <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-3 text-xs text-neutral-700">
                <strong className="text-neutral-800">British Siege (1771):</strong> {heritage.palamuForts.britishSiege}. {heritage.palamuForts.event1857}
              </div>
            </div>
          </div>

          <div className="mt-5 bg-neutral-50 border border-neutral-200 rounded-xl p-4 text-sm text-neutral-700">
        ...
        <section className="bg-black text-white rounded-2xl p-8">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <h3 className="font-serif font-bold text-xl mb-3 text-neutral-200">The Chhotanagpur Plateau</h3>
            ...
            <div>
              <h3 className="font-serif font-bold text-xl mb-3 text-neutral-200">Key Rivers</h3>
              <div className="space-y-2">
                {[
                  ...
                  <div key={name} className="flex items-start gap-2 text-sm border-b border-neutral-800 pb-2">
                    <div>
        ...
        <section id="guidelines">
          <h2 className="font-serif font-bold text-3xl text-neutral-800 mb-4">Visitor Guidelines</h2>
          <p className="text-neutral-600 mb-6">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {[
              { rule: 'Follow forest guide instructions at all times', ok: true },
              { rule: 'Maintain safe distances from all wildlife', ok: true },
              { rule: 'Carry and show valid Government-issued ID at check-in', ok: true },
              { rule: 'Report to reception before 4:00 PM on arrival day', ok: true },
              { rule: 'Littering within reserve boundaries', ok: false },
              { rule: 'Teasing, feeding, or disturbing animals', ok: false },
              { rule: 'Alcohol or drugs within the reserve', ok: false },
              { rule: 'Loud music or noise', ok: false },
              { rule: 'Drones or unmanned aerial vehicles', ok: false },
              { rule: 'Entering the reserve after 4:00 PM', ok: false },
            ].map(({ rule, ok }) => (
              <div key={rule} className={`flex items-start gap-2.5 p-3.5 rounded-xl border text-sm ${
                ok ? 'bg-neutral-50 border-neutral-200 text-neutral-800' : 'bg-red-50 border-red-100 text-red-800'
              }`}>
                {ok
                  ? <CheckCircle size={15} className="text-neutral-600 flex-shrink-0 mt-0.5" />
