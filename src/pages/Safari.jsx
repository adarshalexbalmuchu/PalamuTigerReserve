...
import { Clock, Users, Car, Info, Phone, AlertTriangle, CheckCircle, Ruler } from 'lucide-react'
...
function FeeRow({ label, value, note }) {
  return (
    <div className="flex items-start justify-between gap-4 py-3 border-b border-neutral-100 last:border-0">
      ...
      <div className="text-sm font-bold text-neutral-700 flex-shrink-0">{value}</div>
    </div>
  )
}
    ...
    <div className="min-h-screen bg-cream">
      ...
      <div className="bg-black text-white py-16 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-neutral-300 text-lg font-serif italic mb-6">
          ...
          <div className="flex flex-wrap gap-4">
            {[
              ...
              <div key={label} className="flex items-center gap-2 bg-neutral-900 rounded-lg px-4 py-2.5">
                <span className="text-neutral-400">{icon}</span>
                <div>
                  <div className="text-[10px] text-neutral-400 font-medium uppercase tracking-wider">{label}</div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* Safari Types */}
            <section>
              <h2 className="font-serif font-bold text-2xl text-neutral-800 mb-5">Safari Types</h2>
                ...
                {safariInfo.types.map(t => (
                  <button
                    ...
                    className={`text-left p-6 rounded-2xl border-2 transition-all ${
                      safariType === t.id
                        ? 'border-black bg-neutral-50 shadow-nature'
                        : 'border-neutral-200 bg-white hover:border-neutral-400'
                    }`}
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${safariType === t.id ? 'bg-black' : 'bg-neutral-100'}`}>
                        <Car size={18} className={safariType === t.id ? 'text-white' : 'text-neutral-600'} />
                      </div>
                      {safariType === t.id && <CheckCircle size={18} className="text-black" />}
                    </div>
                    <h3 className="font-serif font-bold text-lg text-neutral-800 mb-1">{t.name}</h3>
                    <p className="text-sm text-neutral-600 mb-3">{t.description}</p>
                    <div className="flex flex-wrap gap-2 text-xs">
                      {t.duration && (
                        <span className="flex items-center gap-1 bg-neutral-50 border border-neutral-200 px-2 py-1 rounded-full text-neutral-700">
                          <Clock size={10} /> {t.duration}
                        </span>
                      )}
                      {t.coverage && (
                        <span className="flex items-center gap-1 bg-neutral-50 border border-neutral-200 px-2 py-1 rounded-full text-neutral-700">
                          <Ruler size={10} /> {t.coverage}
                        </span>
                      )}
                      {t.maxGroupSize && (
                        <span className="flex items-center gap-1 bg-neutral-50 border border-neutral-200 px-2 py-1 rounded-full text-neutral-700">
                          <Users size={10} /> Max {t.maxGroupSize}
                        </span>
                      )}
                    </div>
                    {t.id === 'elephant' && (
                      <div className="mt-3 bg-amber-50 border border-amber-200 rounded-lg p-2 text-xs text-amber-700 flex items-center gap-1.5">
                        <AlertTriangle size={11} /> Max group size: 4 persons per elephant
                      </div>
                    )}
                  </button>
                ))}
            ...
            <section className="bg-white border border-neutral-200 rounded-2xl p-6">
              <h2 className="font-serif font-bold text-2xl text-neutral-800 mb-2">Fee Calculator</h2>
              <p className="text-neutral-500 text-sm mb-6">Configure your safari to get an estimated total</p>

              <div className="grid sm:grid-cols-2 gap-5 mb-6">
                {/* Nationality */}
                <div>
                  <label className="block text-xs font-bold text-neutral-700 uppercase tracking-wider mb-2">Visitor Nationality</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-neutral-700 uppercase tracking-wider mb-2">
                    Group Size: <span className="text-neutral-600 normal-case">{groupSize} {groupSize === 1 ? 'person' : 'persons'}</span>
                  </label>
                  <input
                    type="range"
                    min={1}
                ...
                {/* Guide duration */}
                <div>
                  <label className="block text-xs font-bold text-neutral-700 uppercase tracking-wider mb-2">Guide Duration</label>
                  <div className="grid grid-cols-2 gap-2">
                  ...
                  <p className="text-xs text-neutral-400 mt-1">Guide assignment is mandatory for all safaris</p>
                </div>

                {/* Camera fees */}
                <div>
                  <label className="block text-xs font-bold text-neutral-700 uppercase tracking-wider mb-2">Camera Equipment</label>
                    ...
                    {[
                      ...
                      <label key={key} className="flex items-center gap-2.5 cursor-pointer">
                        <input
                          ...
                          className="w-4 h-4 accent-neutral-800 rounded"
                        />
                        <span className="text-sm text-neutral-700">{label}</span>
                      </label>
                    ))}
                    <p className="text-xs text-neutral-400">Mobile photography free of charge</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-neutral-50 border border-neutral-100 rounded-xl p-5">
                <h3 className="font-semibold text-neutral-800 mb-3">Estimated Fee Breakdown</h3>
                <div>
                  <FeeRow
                  ...
                  {safariType === 'jeep' && (
                    <>
                      <FeeRow label="Safari Package (Jeep)" value={`₹${safariPackage.toLocaleString()}`} note="90 min to 3 hr, 35 km core and buffer" />
                  ...
                  <div className="flex items-center justify-between pt-3 mt-2 border-t-2 border-neutral-200">
                    <span className="font-bold text-neutral-800">Estimated Total (Group)</span>
                    <span className="text-2xl font-bold font-serif text-neutral-800">₹{totalGroup.toLocaleString()}</span>
                  </div>
                  <p className="text-xs text-neutral-400 mt-2">
                    Actual charges may vary. Contact the booking helpline for exact rates.
                  </p>
                </div>
              </div>
            </section>
            ...
            {/* Cancellation Policy */}
            <section className="bg-white border border-neutral-200 rounded-2xl p-6">
              <h2 className="font-serif font-bold text-xl text-neutral-800 mb-4">Safari Cancellation Policy</h2>
              <div className="space-y-3">
                {cancellationPolicy.safari.map((s, i) => (
                  <div key={i} className={`policy-slab ${s.fee === 100 ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200'}`}>
                    ...
                    <div>
                      <div className="font-semibold text-sm text-neutral-800">{s.label}</div>
                      <div className="text-xs text-neutral-600 mt-0.5">{s.condition}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Special Programs */}
            <section>
              <h2 className="font-serif font-bold text-2xl text-neutral-800 mb-5">Special Programmes</h2>
              {safariInfo.specialPrograms.map(prog => (
                <div key={prog.name} className="bg-black rounded-2xl p-6 text-white">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <div className="text-neutral-400 text-xs font-bold uppercase tracking-wider mb-1">Limited Seats</div>
                      <h3 className="font-serif font-bold text-2xl">{prog.name}</h3>
                    </div>
                    <div className="text-right">
                      <div className="text-3xl font-bold font-serif">₹{prog.price.toLocaleString()}</div>
                      <div className="text-neutral-400 text-xs">per explorer</div>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2 mb-4">
                    <span className="bg-neutral-800 text-neutral-200 text-xs px-2.5 py-1 rounded-full">Age: {prog.targetAge}</span>
                    <span className="bg-neutral-800 text-neutral-200 text-xs px-2.5 py-1 rounded-full">Duration: {prog.duration}</span>
                    <span className="bg-neutral-800 text-neutral-200 text-xs px-2.5 py-1 rounded-full">Dates: {prog.dateExample}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    {prog.includes.map(item => (
                      <div key={item} className="flex items-center gap-2 text-sm text-neutral-300">
                        <CheckCircle size={13} className="text-neutral-500 flex-shrink-0" />
                        {item}
                      </div>
                    ))}
                  </div>
          ...
          <div className="space-y-5">
            <div className="bg-black text-white rounded-2xl p-6 sticky top-24">
              ...
              <a
                ...
                className="flex items-center gap-2.5 bg-white hover:bg-neutral-100 text-black rounded-xl p-4 mb-4 transition-colors font-semibold"
              >
                <Phone size={18} />
                <div>
                  <div className="text-xs font-medium text-neutral-500">Booking Helpline</div>
                  <div className="font-bold">{contacts.helpline.display}</div>
                </div>
              </a>

              <div className="space-y-3 border-t border-neutral-800 pt-4 mt-2">
                {[
